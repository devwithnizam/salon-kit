...
class Ajax {
    ...
    public static function init() {
        $actions = [ 'salon_get_services', 'salon_get_professionals', 'salon_get_slots', 'salon_save_booking' ];
    ...
    public static function salon_get_services() {
        $posts = get_posts( [ 'post_type' => 'salon_service', 'posts_per_page' => -1, 'orderby' => 'title', 'order' => 'ASC' ] );
        $data  = [];
        foreach ( $posts as $p ) {
    ...
    public static function salon_get_professionals() {
        $service_id = isset( $_POST['service_id'] ) ? absint( $_POST['service_id'] ) : 0;
        if ( ! $service_id ) wp_send_json_error( [ 'message' => 'No service selected.' ] );

        $pro_ids = (array) get_post_meta( $service_id, '_sb_professionals', true );
        if ( empty( $pro_ids ) ) wp_send_json_success( [] );

        $pros = get_posts( [ 'post_type' => 'salon_professional', 'post__in' => $pro_ids, 'orderby' => 'title', 'order' => 'ASC', 'numberposts' => -1 ] );
        $data = [];
        foreach ( $pros as $pro ) {
            $thumb_id = get_post_thumbnail_id( $pro->ID );
            $data[] = [
                'id'        => $pro->ID,
                'name'      => $pro->post_title,
                'bio'       => get_the_excerpt( $pro ) ?: wp_trim_words( $pro->post_content, 15 ),
    ...
    public static function salon_get_slots() {
        ...
        $slots = Slot_Engine::get_available_slots( $professional_id, $service_id, $date );
    ...
    public static function salon_save_booking() {
        ...
        if ( ! $service_id || ! $professional_id || ! $date || ! $time || ! $name || ! $email ) {
        ...
        delete_transient( "sk_slots_{$professional_id}_{$date}" );
